...
type ColumnType = ColumnDef<AssignmentInterface, unknown>;
...
export function AssignmentListPage(props: AssignmentListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AssignmentInterface>>(
    () => `/assignments?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AssignmentInterface) => {
    if (hasAccess('assignment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/assignments/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'description', header: 'Description', accessorKey: 'description' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.due_date ? format(parseISO(record?.due_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    hasAccess('course', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('grade', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'grade',
          header: 'Grade',
          accessorKey: 'grade',
          cell: ({ row: { original: record } }: any) => record?._count?.grade,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/assignments/view/${record.id}`} passHref legacyBehavior>
                <Button
