...
type ColumnType = ColumnDef<CourseInterface, unknown>;
...
export function CourseListPage(props: CourseListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getCourses({
        relations: ['user', 'assignment.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CourseInterface>>(
    () => `/courses?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('assignment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.assignment,
        }
      : null,
