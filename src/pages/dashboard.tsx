...
import { getGrades } from 'apiSdk/grades';
...
import { AssignmentListPage as ShowCaseList } from 'pages/assignments';

function DashboardPage() {
  const { session } = useSession();

  const gradeStat = useSWR<PaginatedInterface<GradeInterface>>(
    () => `/grades`,
    () => getGrades({ limit: 1, offset: 0 }),
  );
  ...
  const assignmentStat = useSWR<PaginatedInterface<AssignmentInterface>>(
    () => `/assignments`,
    () => getAssignments({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Grades',
      result: gradeStat?.data?.totalCount,
      isLoading: gradeStat.isLoading,
    ...
    {
      title: 'Assignments',
      result: assignmentStat?.data?.totalCount,
      isLoading: assignmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
